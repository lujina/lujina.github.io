...
1、每次仓库repository的改变也就是每一次提交，Git都需要知道是谁创建和提交了它，因此在使用EGit前需要进行配置user.email和user.name。其主要通过<code>preference>Team>Git>Configuration</code>,点击<code>New Entry</code>进行配置。
...
详细EGit用户指导可见[官方文档](http://wiki.eclipse.org/EGit/User_Guide)
